...
include_once(_PS_MODULE_DIR_.'devilslider/DevilSlide.php');

class devilslider extends Module
{
	private $_html = '';

	public function __construct()
	{
		$this->name = 'devilslider';
		...
		$this->displayName = $this->l('Devil Slider for your homepage.');
	...
	{
		/* Adds Module */
		if (parent::install() && $this->registerHook('displayHome') && $this->registerHook('footer') && $this->registerHook('actionShopDataDuplication'))
		{
			/* Sets up configuration */
			$res = Configuration::updateValue('devilslider_WIDTH', '940');
			$res &= Configuration::updateValue('devilslider_HEIGHT', '450');
			$res &= Configuration::updateValue('devilslider_SPEED', '500');
			$res &= Configuration::updateValue('devilslider_PAUSE', '4000');
			$res &= Configuration::updateValue('devilslider_LOOP', '1');
			/* Creates tables */
	...
	public function uninstall()
	{
		...
		if (parent::uninstall())
		{
			...
			$res &= Configuration::deleteByName('devilslider_WIDTH');
			$res &= Configuration::deleteByName('devilslider_HEIGHT');
			$res &= Configuration::deleteByName('devilslider_SPEED');
			$res &= Configuration::deleteByName('devilslider_PAUSE');
			$res &= Configuration::deleteByName('devilslider_LOOP');
			return $res;
		}
		return false;
	}
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'devilslider` (
				`id_devilslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
				`id_shop` int(10) unsigned NOT NULL,
				PRIMARY KEY (`id_devilslider_slides`, `id_shop`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'devilslider_slides` (
			  `id_devilslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
			  `position` int(10) unsigned NOT NULL DEFAULT \'0\',
			  `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
			  PRIMARY KEY (`id_devilslider_slides`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');

		/* Slides lang configuration */
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'devilslider_slides_lang` (
			  `id_devilslider_slides` int(10) unsigned NOT NULL,
			  ...
			  PRIMARY KEY (`id_devilslider_slides`,`id_lang`)
			) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
		');
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new DevilSlide($slide['id_slide']);
			$to_del->delete();
		}
		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'devilslider`, `'._DB_PREFIX_.'devilslider_slides`, `'._DB_PREFIX_.'devilslider_slides_lang`;
		');
	}
	...
	private function _displayForm()
	{
		...
		$this->_html .= '
			...
			<div class="margin-form">
				<input type="text" name="devilslider_HEIGHT" id="speed" size="3" value="'.Tools::safeOutput(Configuration::get('devilslider_HEIGHT')).'" /> px
			</div>';
		/* Width field */
		$this->_html .= '
		<label>'.$this->l('Width:').'</label>
		<div class="margin-form">
			<input type="text" name="devilslider_WIDTH" id="pause" size="3" value="'.Tools::safeOutput(Configuration::get('devilslider_WIDTH')).'" /> px
		</div>';
		/* Speed field */
		$this->_html .= '
			<label>'.$this->l('Speed:').'</label>
			<div class="margin-form">
				<input type="text" name="devilslider_SPEED" id="speed" size="3" value="'.Tools::safeOutput(Configuration::get('devilslider_SPEED')).'" /> ms
			</div>';
		/* Pause field */
		$this->_html .= '
		<label>'.$this->l('Pause:').'</label>
		<div class="margin-form">
			<input type="text" name="devilslider_PAUSE" id="pause" size="3" value="'.Tools::safeOutput(Configuration::get('devilslider_PAUSE')).'" /> ms
		</div>';
		/* Loop field */
		$this->_html .= '
		...
		<div class="margin-form">
			...
			<input type="radio" name="devilslider_LOOP" id="loop_on" '.(Configuration::get('devilslider_LOOP') == 1 ? 'checked="checked"' : '').' value="1" />
			<label class="t" for="loop_on">'.$this->l('Yes').'</label>
			<img src="../img/admin/disabled.gif" alt="No" title="No" style="margin-left: 10px;" />
			<input type="radio" name="devilslider_LOOP" id="loop_off" '.(Configuration::get('devilslider_LOOP') == 0 ? 'checked="checked" ' : '').' value="0" />
	...
	private function _displayAddForm()
	{
		...
		if (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide')))
			$slide = new DevilSlide((int)Tools::getValue('id_slide'));
		...
		foreach ($languages as $language)
		{
			...
			/* Display image */
			if ($slide && $slide->image[$language['id_lang']])
				$this->_html .= '<input type="hidden" name="has_picture" value="1" /><img src="'.__PS_BASE_URI__.'modules/'.$this->name.'/images/'.$slide->image[$language['id_lang']].'" width="'.(Configuration::get('devilslider_WIDTH')/2).'" height="'.(Configuration::get('devilslider_HEIGHT')/2).'" alt=""/>';
			$this->_html .= '</div>';
		}
	...
	private function _postValidation()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			...
			if (!Validate::isInt(Tools::getValue('devilslider_SPEED')) || !Validate::isInt(Tools::getValue('devilslider_PAUSE')) ||
				!Validate::isInt(Tools::getValue('devilslider_WIDTH')) || !Validate::isInt(Tools::getValue('devilslider_HEIGHT')))
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('submitSlider'))
		{
			$res = Configuration::updateValue('devilslider_WIDTH', (int)Tools::getValue('devilslider_WIDTH'));
			$res &= Configuration::updateValue('devilslider_HEIGHT', (int)Tools::getValue('devilslider_HEIGHT'));
			$res &= Configuration::updateValue('devilslider_SPEED', (int)Tools::getValue('devilslider_SPEED'));
			$res &= Configuration::updateValue('devilslider_PAUSE', (int)Tools::getValue('devilslider_PAUSE'));
			$res &= Configuration::updateValue('devilslider_LOOP', (int)Tools::getValue('devilslider_LOOP'));
			$this->clearCache();			
			if (!$res)
		...
		{
			$slide = new DevilSlide((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	{
		if (!$this->isCached('devilslider.tpl', $this->getCacheId()))
		{
			$slider = array(
				'width' => Configuration::get('devilslider_WIDTH'),
				'height' => Configuration::get('devilslider_HEIGHT'),
				'speed' => Configuration::get('devilslider_SPEED'),
				'pause' => Configuration::get('devilslider_PAUSE'),
				'loop' => Configuration::get('devilslider_LOOP'),
			);
			...
			$this->smarty->assign('devilslider_slides', $slides);
			$this->smarty->assign('devilslider', $slider);
		}
	...
	public function hookFooter()
	{
		...
		$this->context->controller->addCSS($this->_path.'css/default.css');
	...
	public function hookDisplayHome()
	{
		...
		return $this->display(__FILE__, 'devilslider.tpl', $this->getCacheId());
	}

	public function clearCache()
	{
		$this->_clearCache('devilslider.tpl');
	}

	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
		INSERT IGNORE INTO '._DB_PREFIX_.'devilslider (id_devilslider_slides, id_shop)
		SELECT id_devilslider_slides, '.(int)$params['new_id_shop'].'
		FROM '._DB_PREFIX_.'devilslider
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
				...
				FROM `'._DB_PREFIX_.'devilslider_slides` hss, `'._DB_PREFIX_.'devilslider` hs
				WHERE hss.`id_devilslider_slides` = hs.`id_devilslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
		);
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hs.`id_devilslider_slides` as id_slide,
					   hssl.`image`,
					   hss.`position`,
					   hss.`active`,
					   hssl.`title`,
					   ...
					   hssl.`legend`,
					   hssl.`description`
			FROM '._DB_PREFIX_.'devilslider hs
			LEFT JOIN '._DB_PREFIX_.'devilslider_slides hss ON (hs.id_devilslider_slides = hss.id_devilslider_slides)
			LEFT JOIN '._DB_PREFIX_.'devilslider_slides_lang hssl ON (hss.id_devilslider_slides = hssl.id_devilslider_slides)
	...
	{
		$req = 'SELECT hs.`id_devilslider_slides` as id_slide
				FROM `'._DB_PREFIX_.'devilslider` hs
				WHERE hs.`id_devilslider_slides` = '.(int)$id_slide;
