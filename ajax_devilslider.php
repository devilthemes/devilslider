...
include_once('devilslider.php');

$context = Context::getContext();
$home_slider = new devilslider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
	{
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'devilslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_devilslider_slides` = '.(int)$id_slide
		);
